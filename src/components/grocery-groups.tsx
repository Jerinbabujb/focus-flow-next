...
import { 
  ...
  inviteMemberAction, 
  removeMemberAction, 
  addSharedGroceryItem 
} from "@/src/actions/grocery-group"
...
export function GroceryGroups({ initialGroups = [], onBack }: GroceryGroupsProps) {
  ...
  const addItem = () => {
    ...
    const qty = Math.max(1, Number.parseInt(itemQty, 10) || 1)
    const amt = itemAmount.trim() === "" ? null : Math.max(0, Number.parseFloat(itemAmount) || 0)
  ...
  const toggleItem = (itemId: string) => {
    ...
    setGroups((prev) => prev.map((g) => g.id === active.id ? { ...g, items: g.items.map((i) => (i.id === itemId ? { ...i, bought: !i.bought } : i)) } : g))
    if (!itemId.startsWith("temp")) toggleGroceryBought(itemId, !item.bought).catch(console.error)
  }

  const deleteItem = (itemId: string) => {
    if (!active) return
    setGroups((prev) => prev.map((g) => g.id === active.id ? { ...g, items: g.items.filter((i) => i.id !== itemId) } : g))
      ...
      <div className="flex flex-col gap-6">
        ...
        <section aria-label="Shared list" className="glass flex flex-col gap-4 rounded-3xl p-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="flex size-9 items-center justify-center rounded-xl bg-secondary text-foreground"><ShoppingCart className="size-4" aria-hidden="true" /></span>
              <h2 className="text-sm font-semibold">Shared list</h2>
            </div>
            <span className="rounded-full bg-primary/15 px-3 py-1 text-sm font-semibold text-primary">{formatMoney(groupTotal)}</span>
          </div>

          <div className="flex flex-col gap-3 rounded-2xl bg-secondary/40 p-3 sm:flex-row sm:items-center">
            <input type="text" value={itemName} onChange={(e) => setItemName(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addItem() }} placeholder="Add to shared list…" className="flex-1 rounded-lg border border-border bg-background/40 px-3 py-2 text-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-ring" />
            <label className="flex items-center gap-2 text-xs text-muted-foreground"><ShoppingBasket className="size-3.5" aria-hidden="true" /> Qty <input type="number" min={1} value={itemQty} onChange={(e) => setItemQty(e.target.value)} className="w-16 rounded-lg border border-border bg-background/40 px-2.5 py-2 text-sm outline-none focus:ring-2 focus:ring-ring" /></label>
            <label className="flex items-center gap-2 text-xs text-muted-foreground">Amount <input type="number" min={0} step="0.01" value={itemAmount} onChange={(e) => setItemAmount(e.target.value)} placeholder="0.00" className="w-24 rounded-lg border border-border bg-background/40 px-2.5 py-2 text-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-ring" /></label>
            <button type="button" onClick={addItem} className="flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90"><Plus className="size-4" aria-hidden="true" /> Add</button>
          </div>

          {active.items.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">No shared items yet. Add something for the group to pick up.</p>
          ) : (
            <ul className="flex flex-col gap-2">
              {active.items.map((item) => (
                <li key={item.id} className="flex items-center gap-3 rounded-2xl bg-secondary/40 p-3">
                  <button type="button" onClick={() => toggleItem(item.id)} className={cn("flex size-6 shrink-0 items-center justify-center rounded-md border transition-colors", item.bought ? "border-primary bg-primary text-primary-foreground" : "border-border text-transparent hover:border-primary hover:text-primary")}><Check className="size-4" aria-hidden="true" /></button>
                  <div className="flex flex-1 flex-col gap-1">
                    <span className={cn("text-sm font-medium", item.bought && "text-muted-foreground line-through")}>{item.name}</span>
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1"><ShoppingBasket className="size-3" aria-hidden="true" /> {item.quantity}</span>
                      {item.amount != null && <span>{formatMoney(item.amount)}</span>}
                      <span>added by {item.addedBy}</span>
                    </div>
                  </div>
                  <button type="button" onClick={() => deleteItem(item.id)} className="flex items-center justify-center rounded-lg border border-border p-2 text-muted-foreground transition-colors hover:bg-destructive/15 hover:text-destructive"><Trash2 className="size-4" aria-hidden="true" /></button>
                </li>
              ))}
            </ul>
          )}
  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-3">
        <button type="button" onClick={onBack} className="flex w-fit items-center gap-2 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground"><ArrowLeft className="size-4" aria-hidden="true" /> Back to groceries</button>
        <div>
      ...
      <section aria-label="Create group" className="glass flex flex-col gap-4 rounded-3xl p-5">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <input type="text" value={newGroupName} onChange={(e) => setNewGroupName(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") createGroup() }} placeholder="Group name, e.g. Family, Roommates…" className="flex-1 rounded-lg border border-border bg-background/40 px-3 py-2 text-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-ring" />
      ...
      <section aria-label="Your groups" className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {groups.length === 0 ? (
          <div className="glass col-span-full rounded-3xl p-8 text-center text-sm text-muted-foreground">You&apos;re not in any groups yet. Create one above to start shopping together.</div>
        ) : (
          groups.map((g) => {
